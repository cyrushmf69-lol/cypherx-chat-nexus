...
import { validateEmail, loginRateLimiter } from '@/utils/security';
...
const AuthContainer = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isLogin && !loginRateLimiter.isAllowed(formData.email)) {
      const remainingTime = Math.ceil(loginRateLimiter.getRemainingTime(formData.email) / 1000 / 60);
      toast({
        ...
        <Card className="p-6 backdrop-blur-sm bg-white/80 border-0 shadow-xl">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Button 
              type="submit" 
              disabled={loading}
              className="w-full h-12 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white font-medium text-lg transition-all duration-200 transform hover:scale-105 disabled:opacity-50 disabled:transform-none"
            >
              {loading ? (
            ...
            <div className="grid grid-cols-3 gap-3">
              <Button variant="outline" type="button" disabled={loading} className="h-12 border-gray-200 hover:bg-gray-50">
